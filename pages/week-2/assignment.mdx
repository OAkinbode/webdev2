...
In the `week-2` folder, create a new **file** called `page.tsx`.

In the `week-2/page.tsx` file, enter the following code to display a heading:
...
In the `week-2` folder, create a file called `student.info.tsx`.

In the `student.info.tsx` file, code a React component called `StudentInfo` that renders (displays) the following information:
...
In the `week-2/page.tsx` file, import the `StudentInfo` component you created in the previous step.
...
The root page is the page that is displayed when you navigate to `http://localhost:3000/`. It is the `page.tsx` file in the project `/app` folder.
...
Deploy what you built to vercel. Follow this link as a guide: See [Guide: Deploying a Next.js Application](/guides-and-references/deploying-nextjs-app) for instructions.
...
- The weekly folder is named exactly `week-2` (lowercase) and contains the `page.tsx` file.
