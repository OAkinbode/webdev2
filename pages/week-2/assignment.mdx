...
In the `week-2` folder, create a new **file** called `page.ts`.

In the `week-2/page.ts` file, enter the following code to display a heading:
...
In the `week-2` folder, create a file called `student-info.ts`.

In the `student-info.ts` file, code a React component called `StudentInfo` that renders (displays) the following information:
...
In the `week-2/page.ts` file, import the `StudentInfo` component you created in the previous step.
...
The root page is the page that is displayed when you navigate to `http://localhost:3000/`. It is the `page.ts` file in the project `/app` folder.
...
- The weekly folder is named exactly `week-2` (lowercase) and contains the `page.ts` file.
