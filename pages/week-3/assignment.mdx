...
Edit the project file `/app/page.ts` and add a link to `week-3` in the list of links.
...
Create the folder `/app/week-3` and create files `item.tsx`, `item-list.tsx`, and `page.tsx` inside it.

### Build the `Item` Component

In `item.tsx`, create a functional component named `Item`. This component should accept `name`, `quantity`, and `category` as props and display them in a list item element. Use Tailwind classes for styling. Make sure that you use Typescript interface if you are using Typescript.

### Build the `ItemList` Component

In `item-list.tx`, create a functional component named `ItemList`. Inside this component, copy paste the following item objects each with `name`, `quantity`, and `category` properties. Render these items using the `Item` component you just created, passing item data as props.
...
In `page.tsx`, create a functional component named `Page` that returns a `main` element wrapped around an `h1` "Shopping List" header and the `ItemList` component. Use Tailwind classes for styling.

### Update Page Title

In the file `/app/layout.tsx`, edit the `title` property of the `metadata` object to display "Shopping List" as the browser document title.
