...
Edit the project file `/app/page.tsx` and add a link to `week-5` in the list of links.

### Create the Folder and Files

Create the folder `/app/week-5` and the files `page.tsx`, `item-list.tsx`, and `item.tsx` inside it. If you completed the Week 3 Assignment, you can copy the files from that assignment into this folder.
...
> In `item.tsx`, create a functional component named `Item`. This component should accept `name`, `quantity`, and `category` as props and display them in a list item element.

### Build the `Page` Component

No change from the Week 3 Assignment.

> In `page.tsx`, create a functional component named `Page` that returns a `main` element wrapped around an `h1` "Shopping List" header and the `ItemList` component.
