...
`reduce` is a very powerful method that can be used to solve a wide range of problems.
...
Week 5 of the Community Events app demonstrates how to display a list of event objects. Focus on how the `EventsList` component uses the `map()` method to render a list of `EventCard` components.
